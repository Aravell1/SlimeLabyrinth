...
public class GridManager : MonoBehaviour
{
    ...
    [SerializeField]
    float gridCellSize = 5;
    [SerializeField]
    bool showGrid = true;
    [SerializeField]
    bool showObstacleBlocks = true;
    ...
    private void InitializeNodes() 
    {
        ...
        for (int i = 0; i < numberOfColumns; i++) 
        {
            for (int j = 0; j < numberOfRows; j++) 
            {
                ...
                Node node = new(cellPosition);
                if (Physics.Raycast(cellPosition, Vector3.down, 2))
    ...
    private void CreateObstacle(Vector3 cellPosition)
    {
        GameObject obstacle = Instantiate(obstaclePrefab, cellPosition, transform.rotation);
